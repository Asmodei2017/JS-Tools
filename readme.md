...
`search(regexp)`          выполняет поиск сопоставления между регулярным выражением и строкой. Если аргументом не передать рег.выражение то оно создастся автоматически. ВОЗВРАЩАЕТ ИНДЕКС сивола в строке ИЛИ -1 если такого символа нет
...
`.filter(callbac(x,y,z))`  создаёт НОВЫЙ массив со всеми элементами, прошедшими проверку. 
Метод filter() НЕ ИЗМЕНЯЕТ массив, для которого он был вызван.
Проверяет каждый елемент массива, и если он удовлетворяет НАШЕМУ условию, записывает его в НОВЫЙ МАССИВ. Параметры: X - елемент массива, Y - индекс массива, Z - сам массив
